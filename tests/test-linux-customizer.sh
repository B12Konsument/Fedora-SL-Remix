...
jq -n \
    ...
    --arg bundle_hash "$zero_hash" --argjson base_size "$base_size" '{
      schema:1,minimum_customizer_version:"0.2.5",fedora_release:44,remix_version:"0.2.7",
      ...
      linux_bundle:{name:"linux-customizer.tar.gz",size:123,sha256:$bundle_hash,minimum_customizer_version:"0.2.7",customizer_version:"0.2.7",entrypoint:"linux/new-fedora-sl7-iso.sh"}
    }' >"$fixture/layout.json"
jq -n --argjson base_size "$base_size" '{
      tag_name:"v0.2.7",draft:false,assets:[
...
jq -n \
    ...
    --arg msi_hash "$msi_hash" --arg bundle_hash "$bundle_hash" --argjson msi_size "$msi_size" --argjson bundle_size "$bundle_size" '{
      schema:1,minimum_customizer_version:"0.2.5",fedora_release:44,remix_version:"0.2.7",source_lock_sha256:("1"*64),
      ...
      linux_bundle:{name:"linux-customizer.tar.gz",size:$bundle_size,sha256:$bundle_hash,minimum_customizer_version:"0.2.7",customizer_version:"0.2.7",entrypoint:"linux/new-fedora-sl7-iso.sh"}}
    ' >"$e2e/layout.json"
layout_size=$(stat -c %s "$e2e/layout.json")
jq -n \
    --arg base "file://$e2e_url/base.iso" --arg bundle "file://$e2e_url/linux-customizer.tar.gz" \
    --arg layout "file://$e2e_url/layout.json" --argjson bundle_size "$bundle_size" --argjson layout_size "$layout_size" '{tag_name:"v0.2.7",draft:false,assets:[
...
expect_failure 'placeholder hash' env PATH="$e2e/fakebin:$PATH" HOME="$e2e/home" TMPDIR="$e2e/tmp" \
    ...
    "$root/linux/new-fedora-sl7-iso.sh" --non-interactive --release v0.2.7 --model romulus15 \
...
final_iso=$e2e/output/Fedora-SL7-Remix-44-0.2.7-Romulus15-PRIVATE.aarch64.iso
